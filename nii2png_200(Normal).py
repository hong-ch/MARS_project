...
def main():
    ...
    input_dir = os.path.join(project_root, "nifti", "Normal")
    output_base_dir = os.path.join(project_root, "LDCT_PNG_200", "Normal")
    ...
    # Define the range of slices to extract (100 before and 100 after middle slice)
    slice_range = 100
    ...
    for nii_file in nii_files:
        ...
        # Call nii2png.py with the input file, output directory, and slice range
        try:
            # Assuming nii2png.py accepts --start and --end arguments for slice range
            # Adjust these arguments based on your nii2png.py implementation
            # Here, we calculate the middle slice and extract ±100 slices
            subprocess.run([
                "python", "nii2png.py",
                "-i", nii_file,
                "-o", output_dir,
                "--start", str(slice_range * -1),  # -100
                "--end", str(slice_range)          # +100
            ], check=True)
