...
def png_to_nifti(source_dir, dest_dir):
    """
    Converts directories of PNG slices into NIfTI files.
    Assumes source_dir contains class folders, which in turn contain patient folders with PNG slices.
    """
    ...
    for class_name in tqdm(class_folders, desc="Processing classes"):
        ...
        patient_folders = sorted([d for d in os.listdir(class_path) if os.path.isdir(os.path.join(class_path, d))])

        for patient_id in tqdm(patient_folders, desc=f"Processing patients in {class_name}", leave=False):
            ...
            if not png_files:
                print(f"Warning: No PNG files found for {patient_id} in {class_name}. Skipping.")
                continue
            ...
            try:
                ...
                # Save the NIfTI file
                output_filename = os.path.join(dest_class_path, f"{patient_id}.nii.gz")
...
if __name__ == '__main__':
    ...
    SOURCE_DATA_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'data', 'ULDCT_png'))
    DEST_DATA_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'data', 'ULDCT_nifti'))
