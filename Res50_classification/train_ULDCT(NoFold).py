...
num_classes = 3
dataset_root = 'C:/Users/smc/Desktop/CH/MARS_project/data/ULDCT_png_image_100(cropped)_train'
test_dataset_root = 'C:/Users/smc/Desktop/CH/MARS_project/data/ULDCT_png_image_100(cropped)_test'  # 테스트 데이터 폴더 경로
...
def train_model(model, train_loader, val_loader, epochs):
    ...
    # TensorBoard 로그 디렉토리
    log_dir = f"runs/ver17_NoFold-ULDCT"
    writer = SummaryWriter(log_dir=log_dir)
    
    # Best model 저장 경로
    checkpoint_dir = os.path.join(log_dir, 'checkpoint')
    os.makedirs(checkpoint_dir, exist_ok=True)
    best_model_path = os.path.join(checkpoint_dir, 'weights-best_model.pth')
    ...
    writer.close()
    pd.DataFrame(history).to_csv(f'{Disease_name}-History.csv', index=False)
    return model
...
if __name__ == '__main__':
    ...
    # 인덱스 분할 (Train:Val:Test = 8:1:1)
    indices = list(range(len(full_dataset)))
    train_indices, val_indices = train_test_split(indices, train_size=0.8889, random_state=42)  # 0.8 / 0.9 ≈ 0.8889

    # 서브셋 생성 및 transform 적용
    train_dataset = CustomCTScanDataset(dataset_root, transform=train_transform)
    train_dataset.patient_folders = [full_dataset.patient_folders[i] for i in train_indices]
    train_dataset.labels = [full_dataset.labels[i] for i in train_indices]
    val_dataset = CustomCTScanDataset(dataset_root, transform=val_transform)
    val_dataset.patient_folders = [full_dataset.patient_folders[i] for i in val_indices]
    val_dataset.labels = [full_dataset.labels[i] for i in val_indices]
    # 테스트 데이터셋 로드 (별도 폴더, 30명)
    ...
    print(f'학습 환자: {len(train_indices)}, 검증 환자: {len(val_indices)}, 테스트 환자: {len(test_dataset)}')
    ...
    test_probs = []
    with torch.no_grad():
        test_bar = tqdm(test_loader, desc="테스트 평가")
    ...
    plt.tight_layout()
    plt.savefig('confusion_matrix_test(Low-Dose).png')
    plt.close()

    print(f"Test Confusion Matrix:\n{test_cm}")

    # Best model 저장
    checkpoint_dir = os.path.join(f"runs/ver17_NoFold-ULDCT", 'checkpoint')
    os.makedirs(checkpoint_dir, exist_ok=True)
    best_model_path = os.path.join(checkpoint_dir, 'weights-best_model.pth')
