...
batch_size = 4
num_classes = 3
test_dataset_root = 'C:/Users/smc/Desktop/CH/MARS_project/data/ULDCT_png_image_100(cropped)_test'  # Test 데이터 폴더 경로
checkpoint_path = 'runs/ver1_LDCT/checkpoint/weights-best_model.pth'  # 학습된 가중치 경로
...
def inference(model, test_loader):
    ...
    with torch.no_grad(), autocast('cuda'):
        test_bar = tqdm(test_loader, desc="Inference on LDCT")
...
if __name__ == '__main__':
    ...
    checkpoint_path = 'runs/ver1_LDCT/checkpoint/weights-best_model.pth'
    ...
    plt.title('ROC Curve (LDCT)')
    plt.legend(loc='best')
    plt.savefig('roc_curve_LDCT.png')
    plt.close()
    ...
    plt.title('Confusion Matrix (LDCT)')
    plt.colorbar()
    ...
    plt.tight_layout()
    plt.savefig('confusion_matrix_LDCT.png')
    plt.close()
